...
class StreamingTranscriber:
    ...
    def process_iter(self) -> StreamingResult:
        if self.buffered_seconds < MIN_SECONDS_TO_PROCESS:
            return StreamingResult(
                ...
                pending_text=sanitize_transcription(self._words_to_text(self.previous_tail), self.language),
        ...
        newly_committed = ""
        if common_prefix:
            raw_committed = self._words_to_text(common_prefix)
            newly_committed = sanitize_transcription(raw_committed, self.language)
    ...
    def finalize(self) -> StreamingResult:
        ...
        Why: on stop we must transcribe whatever audio is left, no matter how short.
        Skipping sub-second tails drops the last word the user spoke right before clicking.
        """
        if self.buffered_seconds > 0:
            ...
            if tail_text:
                self.confirmed_text += sanitize_transcription(tail_text, self.language)
            elif self.previous_tail:
                self.confirmed_text += sanitize_transcription(self._words_to_text(self.previous_tail), self.language)
        elif self.previous_tail:
            self.confirmed_text += sanitize_transcription(self._words_to_text(self.previous_tail), self.language)

        self.confirmed_text = sanitize_transcription(self.confirmed_text, self.language)
